...
上游服务投递 MinerU PDF 解析任务时，请参考：

- `docs/producer-interface.md`

服务启动后，也可以访问运行时 HTTP API 文档：
...
本服务默认让每个 Celery Worker 同时只执行 1 个 MinerU PDF 解析任务，并在启动 MinerU 子进程前根据机器 CPU 与可用内存自动设置内部并行参数。详细配置与排查方式请参考：

- `docs/mineru-tuning.md`
